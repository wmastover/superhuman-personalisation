...
export function UploadStep({ onParsed, onLoadSession, onDeleteSession }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-6 p-12 overflow-y-auto">
      ...
      {(() => {
        const activeSessions = sessions.filter(s => s.step !== 'export');
        const archivedSessions = sessions.filter(s => s.step === 'export');

        const renderSession = (session: SavedSession, isArchived: boolean) => {
          ...
          return (
            ...
            >
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${isArchived ? 'bg-green-50' : 'bg-gray-100'}`}>
                {isArchived ? (
            ...
            {archivedSessions.length > 0 && (
              <div className="w-full max-w-lg">
                <button
                  ...
                  className="flex items-center gap-2 text-xs font-medium text-gray-400 uppercase tracking-wide mb-2 hover:text-gray-600 transition-colors w-full text-left"
                >
                  <svg
                    className={`w-3.5 h-3.5 transition-transform ${archivedExpanded ? 'rotate-90' : ''}`}
